...
try {
  ...
  if (obj.data && Array.isArray(obj.data)) {
    ...
    obj.data = obj.data.filter(item => {
      ...
      // 【新增】过滤 type === 'ad' 类型的广告（next-render中的广告）
      if (item.type === 'ad') {
        console.log('已过滤: ad 类型广告');
        return false;
      }

      // 过滤包含广告标识的卡片
      if (item.ad || item.adjson) {
        console.log('已过滤: 包含ad/adjson字段的广告');
        return false;
      }
      ...
      // 过滤 commercial_card 和 promotion 类型
